# read the list of urls, read, confirm column format, validate and when using in development mode, make sure do run `devtools::load_all()` to get all the CollaboratoR functions loaded save to CSV

read the list of urls, read, confirm column format, validate and when
using in development mode, make sure do run
[`devtools::load_all()`](https://devtools.r-lib.org/reference/load_all.html)
to get all the CollaboratoR functions loaded save to CSV

## Usage

``` r
validate_all(urls.df = NULL, drive_email = NULL)
```
